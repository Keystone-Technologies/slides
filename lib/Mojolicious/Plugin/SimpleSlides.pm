...
<html>
  <head>
    ...
    % if ( my $href = simple_slides->base_href ) {
      %= tag base => href => $href
    % }
    % if ( my $ppi_css = __PACKAGE__->can('ppi_css') ) {
      %= $ppi_css->();
    % }
    ...
    % end
    %= javascript '//simonwaldherr.github.io/micromarkdown.js/micromarkdown.js'
  </head>
  <body>
    <div class="container">
...
%= content
    ...
    %= javascript begin
      document.getElementById('main').innerHTML = micromarkdown.parse(document.getElementById('slide-content').innerHTML);
    %= end
  </body>
</html>
